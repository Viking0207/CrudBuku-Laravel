    ...
    <form action="/admin/buku/{{ $buku->id }}" method="POST" enctype="multipart/form-data"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="md:col-span-2">
                <label class="block mb-1 font-semibold text-gray-700" for="gambar">
                    <i class="fa-solid fa-image mr-2 text-pink-600"></i> Gambar Buku (opsional)
                </label>

                @if ($buku->gambar)
                    <div class="mb-3">
                        <img src="{{ asset('gambar_buku/' . $buku->gambar) }}" alt="Gambar Buku Lama"
                            class="w-32 h-auto rounded shadow border border-gray-300" />
                        <p class="text-sm text-gray-600 mt-1">Gambar saat ini. Jika tidak ingin mengganti, kosongkan
                            input di bawah.</p>
                    </div>
                @endif

                <input type="file" id="gambar" name="gambar" accept="image/*"
                    class="border border-gray-300 p-2 rounded w-full focus:outline-none focus:ring-2 focus:ring-pink-400" />
            </div>
        </div>
